...
def basic_encoder() -> RocketQADualEncoder:
    """
    dual encoder running on CPU
    :return: dual encoder running on CPU
    """
    return RocketQADualEncoder(name="zh_dureader_de_v2", use_cuda=False)
...
def test_encoding_cpu():
    ...
    """
    docs = DocumentArray([Document(text="关山难越，谁悲失路之人")])
    basic_encoder_cpu = RocketQADualEncoder(name="zh_dureader_de_v2", use_cuda=False)
    basic_encoder_cpu.encode_passage(docs)
